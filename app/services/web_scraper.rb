...
def fetch_page(url)
  begin
    ...
    if response.success?
      ...
      puts "Failed to fetch #{url}. HTTP Status: #{response.code}"
      puts "Response Body: #{response.body[0..500]}"  # Output only the first 500 characters of the response body
      ""
    end
  rescue HTTParty::Error => e
    # Catch and log any HTTParty-specific errors
    puts "HTTParty Error while fetching #{url}: #{e.message}"
    ""
...
def amazon_scrape_search_page(initial_url, max_pages = 2)
  ...
  while pages_to_scrape.length != 0 && i < max_pages do
  ...
  CSV.open(products_filename, "wb", write_headers: true, headers: ["url", "image", "name", "brand", "price"]) do |csv|
    products.each { |product| csv << product }
  end

  puts "Search page scraping completed. Check the '#{products_filename}' file for results."
end
...
def amazon_add_additional_images(input_filename)
  ...
  search_term = search_term_sanitized.gsub('_', ' ')  # Restore original search term for use in output filename
  ...
  CSV.foreach(input_filename, headers: true) do |row|
    ...
    print "\rProcessing: #{percentage}% complete"
  end

  # Write the updated products with additional images to a new CSV file
  CSV.open(output_filename, "wb", write_headers: true, headers: ["url", "image", "name", "brand", "price", "additional_images"]) do |csv|
    products.each do |product|
      csv << [product[:url], product[:image], product[:name], product[:brand], product[:price], product[:additional_images]]
    end
  end

  puts "Additional images added. Check the '#{output_filename}' file for results."
end
